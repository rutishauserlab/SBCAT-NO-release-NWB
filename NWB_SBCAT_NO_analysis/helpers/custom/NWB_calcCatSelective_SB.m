function [sig_cells,areasSternberg] = NWB_calcCatSelective_SB(nwbAll, all_units, params)
...
pref_cat = zeros(length(all_units),1); 
...
for i = 1:length(all_units) 
    ...
    % Loading stim timestamps and loads from WM part
    tsFix = num2cell(nwbAll{SU.session_count}.intervals.get('WM_trials').vectordata.get('timestamps_FixationCross').data.load());
    tsEnc1 = num2cell(nwbAll{SU.session_count}.intervals.get('WM_trials').vectordata.get('timestamps_Encoding1').data.load());
    tsEnc2 = num2cell(nwbAll{SU.session_count}.intervals.get('WM_trials').vectordata.get('timestamps_Encoding2').data.load());
    tsEnc3 = num2cell(nwbAll{SU.session_count}.intervals.get('WM_trials').vectordata.get('timestamps_Encoding3').data.load());
    tsMaint = num2cell(nwbAll{SU.session_count}.intervals.get('WM_trials').vectordata.get('timestamps_Maintenance').data.load());
    tsProbe = num2cell(nwbAll{SU.session_count}.intervals.get('WM_trials').vectordata.get('timestamps_Probe').data.load());
    ID_Enc1 = num2cell(nwbAll{SU.session_count}.intervals.get('WM_trials').vectordata.get('PicIDs_Encoding1').data.load());
    ID_Enc2 = num2cell(nwbAll{SU.session_count}.intervals.get('WM_trials').vectordata.get('PicIDs_Encoding2').data.load());
    ID_Enc3 = num2cell(nwbAll{SU.session_count}.intervals.get('WM_trials').vectordata.get('PicIDs_Encoding3').data.load());
    ID_Probe = num2cell(nwbAll{SU.session_count}.intervals.get('WM_trials').vectordata.get('PicIDs_Probe').data.load());
...
%     nonPar_method = 'other'; % Prior selectivity method. Not same used in Daume et al
    ...
    elseif strcmp(sig_method,'non-parametric')
        ...
        if strcmp(nonPar_method,'statcond') % Statcond version (From Daume et al)
            ...
            [F_ANOVA, ~, p_ANOVA] = statcond(D,'paired','off','method', 'perm', 'naccu', 10000, 'verbose','off');
    ...
    if plotFlag || params.plotAlways
        ...
        ProbeInOut = nwbAll{SU.session_count}.intervals.get('WM_trials').vectordata.get('probe_in_out').data.load();
